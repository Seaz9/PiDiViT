...
from lib.regionprop_update import augment_rois, region_coord_2_abs_coord, abs_coord_2_region_coord, SpatialIntegral, \
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.training:
            ...
            aug_rois, init_region, gt_region, _ = augment_rois(rois[:, 1:], matched_gt_boxes, img_h=H, img_w=W,
                                                               pooler_size=self.roialign_size,
            ...
            aug_rois, init_region, _, _ = augment_rois(rois[:, 1:], None, img_h=H, img_w=W,
                                                       pooler_size=self.roialign_size,
                                                       min_expansion=0.4, expand_shortest=True)
            aug_rois = torch.cat([rois[:, :1], aug_rois], dim=1)
        ...
        projected_feats = self.foreground_linears['feat'](roi_features.transpose(-2, -1)).permute(0, 2, 1).reshape(bs,
                                                                                                                   -1,
                                                                                                                   self.roialign_size,
                                                                                                                   self.roialign_size)
        ...
        bg_dist_emb = bg_dist_emb.permute(0, 2, 1).reshape(bs, -1, self.roialign_size, self.roialign_size)
        projected_feats = self.background_linears['feat'](roi_features.transpose(-2, -1)).permute(0, 2, 1).reshape(bs,
                                                                                                                   -1,
                                                                                                                   self.roialign_size,
                                                                                                                   self.roialign_size)
        ...
        if self.training:
            ...
            pred_deltas = self.box2box_transform.get_deltas(
                rois[:, None, 1:].repeat(1, num_active_classes, 1).flatten(0, 1), pred_abs_boxes)
        ...
        if self.training:
            ...
            if self.use_one_shot:
                ...
                for target_cid in target_class_ids:
                    # print("class_indices:", class_indices, type(class_indices))
                    # print("target_cid:", target_cid, type(target_cid))
