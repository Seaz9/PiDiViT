...
def cosine_annealing_gamma(epoch, num_epochs, initial_gamma=0.5, final_gamma=2.0):
    """
    使用余弦退火动态调整gamma值。
    """
    cos_inner = (math.pi * epoch) / num_epochs
    cos_out = math.cos(cos_inner) + 1
    gamma = final_gamma - 0.5 * (final_gamma - initial_gamma) * cos_out
    return gamma

def focal_loss(inputs, targets, num_epochs=900, reduction="mean", bg_weight=0.2, num_classes=None, initial_gamma=0.5, final_gamma=2.0):
    """
    Focal loss with dynamic gamma adjusted using cosine annealing.
    """
    # 使用静态变量跟踪并自动递增epoch
    if not hasattr(focal_loss, "epoch"):
        focal_loss.epoch = 0  # 初始化epoch为0

    # 动态计算当前epoch的gamma值
    gamma = cosine_annealing_gamma(focal_loss.epoch, num_epochs, initial_gamma, final_gamma)

    # 每次调用函数后增加epoch
    focal_loss.epoch = (focal_loss.epoch + 1) % num_epochs  # 使epoch在[0, num_epochs)间循环

    # 如果目标标签为空，直接返回损失为0
    if targets.numel() == 0 and reduction == "mean":
        return inputs.sum() * 0.0  # 保持梯度连接
    ...
    p_t = p[torch.arange(p.size(0)).to(p.device), targets]  # 获取目标类别的概率
    p_t = torch.clamp(p_t, 1e-7, 1 - 1e-7)  # 防止NaN情况

    # 计算带有focal scaling的损失
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if (self.training and (not self.only_train_mask)) or (not self.training):
            ...
            intra_dist_emb = distance_embed(intra_feats.flatten(0, 1),
                                            num_pos_feats=self.Tpos_emb)  # (Nxspatial) x class x emb TODO: 可以再加一个 linear 层这里
